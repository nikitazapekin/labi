...
namespace lab7Var12
{
    public partial class MainWindow : Window
    {
        ...
        private void SearchNote_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                IEnumerable<NOTE> searchResults = notes.Where(note =>
                    (searchCriteria == "По дате" && DateTime.TryParseExact(searchText, "dd/MM/yyyy", null, System.Globalization.DateTimeStyles.None, out DateTime searchDate) &&
                        new DateTime(note.BirthDate[2], note.BirthDate[1], note.BirthDate[0]) == searchDate) ||
                    (searchCriteria == "По телефону" && note.PhoneNumber.Equals(searchText, StringComparison.OrdinalIgnoreCase)) ||
            ...
            {
                MessageBox.Show($"Ошибка при обработке поиска: {ex.Message}", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void SortNotes_Click(object sender, RoutedEventArgs e)
        {
            ...
            IEnumerable<NOTE> sortedNotes = sortCriteria switch
            {
                "По имени" => notes.OrderBy(note => note.FullName, StringComparer.OrdinalIgnoreCase),
                "По дате" => notes.OrderBy(note => new DateTime(note.BirthDate[2], note.BirthDate[1], note.BirthDate[0])),
                "По телефону" => notes.OrderBy(note => note.PhoneNumber, StringComparer.OrdinalIgnoreCase),
                _ => notes
            };

            notes = sortedNotes.ToList();
            UpdateNoteList();
        }
        ...
        private void DeleteNote_Click(object sender, RoutedEventArgs e)
        {
            ...
            var noteToDelete = notes.FirstOrDefault(note => note.FullName.Equals(fullNameToDelete, StringComparison.OrdinalIgnoreCase));
            if (noteToDelete.Equals(default(NOTE)))
            {
        ...
        private void AddDefaultNotes_Click(object sender, RoutedEventArgs e)
        {
            ...
            notes.AddRange(defaultNames.Select((name, index) =>
            {
                string[] dateParts = defaultDates[index].Split('/');
                return new NOTE
                {
                    FullName = name,
                    PhoneNumber = defaultPhones[index],
